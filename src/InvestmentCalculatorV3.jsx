...
function getExpenseAmount(expense, totalIncomeAfterTax) {
  if (expense.mode === "percent") {
    return Math.max(0, totalIncomeAfterTax) * (clamp(toNum(expense.amount), 0, 100) / 100);
  }
...
function getManualPayoutUsedAmount(payoutMode, payoutDollarAmount, payoutPercent, netAfterTax) {
...
function deployedPrincipal(state) {
  if (!state) return 0;
  return state.contracts.reduce((sum, contract) => sum + Math.max(0, toNum(contract.principal)), 0);
}
...
export default function InvestmentCalculatorV3() {
  ...
  function getEffectivePayoutUsed(netAfterTax, expenseTotal) {
    if (autoCoverShortfallEnabled) {
      const wageOnlyShortfall = Math.max(0, expenseTotal - incomeWithoutPayout);
      return Math.min(Math.max(0, netAfterTax), wageOnlyShortfall);
    }

    return getManualPayoutUsedAmount(payoutMode, payoutUsedMonthly, payoutPercent, netAfterTax);
  }

  function calculateMonthlyCashFlow(activePrincipal) {
    const payout = calculatePayout(activePrincipal);
    const totalSpendableIncome = incomeWithoutPayout + payout.netAfterTax;
    const expenseTotal = getExpenseTotal(expenses, totalSpendableIncome);
    const payoutUsed = getEffectivePayoutUsed(payout.netAfterTax, expenseTotal);
    const totalSurplus = Math.max(0, totalSpendableIncome - expenseTotal);
    const leftoverFromPayout = Math.max(0, payout.netAfterTax - payoutUsed);
    ...
    switch (reinvestMode) {
      case "reinvest_all_total":
        toReinvest = totalSurplus;
        break;
      case "reinvest_net_only":
        toReinvest = Math.min(totalSurplus, leftoverFromPayout);
        break;
      case "reinvest_fixed":
        toReinvest = Math.min(totalSurplus, fixedTarget);
        break;
      case "reinvest_pct_of_payout":
        toReinvest = Math.min(totalSurplus, payout.netAfterTax * percentageTarget);
        break;
      case "reinvest_pct_of_total_surplus":
        toReinvest = totalSurplus * percentageTarget;
        break;
      default:
        toReinvest = 0;
    }

    const safeReinvest = Math.max(0, Math.min(totalSurplus, toReinvest));
    ...
    return {
      ...
      totalSurplus,
      toReinvest: safeReinvest,
      cashRemaining: Math.max(0, totalSurplus - safeReinvest)
    };
  }

  function simulateNonLadderMonth(currentPrincipal) {
    const cashFlow = calculateMonthlyCashFlow(currentPrincipal);
    const endingPrincipal = currentPrincipal + cashFlow.toReinvest;

    return {
      nextPrincipal: endingPrincipal,
      payout: cashFlow.payout,
      cashFlow,
      activePrincipal: currentPrincipal,
      totalCapital: endingPrincipal,
      savedPool: 0,
      contracts: 1,
      newDeployment: cashFlow.toReinvest,
      renewalDeployment: 0
    };
  }

  function simulateLadderMonth(currentState) {
    const nextState = cloneLadderState(currentState) || createInitialLadderState(principal);
    ...
    const cashFlow = calculateMonthlyCashFlow(activePrincipal);
    nextState.savedPool += cashFlow.toReinvest;

    const calendarSlot = nextState.elapsedMonths % 12;
    const slotOccupied = nextState.contracts.some((contract) => contract.slotMonth === calendarSlot);
    let newDeployment = 0;

    if (!renewedThisMonth && !slotOccupied && nextState.savedPool >= MINIMUM_NEW_CONTRACT) {
  ...
  const activePrincipalForDisplay = ladderEnabled && ladderState ? deployedPrincipal(ladderState) : principal;
  const payoutNow = useMemo(
    () => calculatePayout(activePrincipalForDisplay),
    [
      activePrincipalForDisplay,
      monthlyRate,
      filingStatus,
      monthlyIncome,
      otherIncome,
  ...
  const payoutUsedInput = getEffectivePayoutUsed(payoutNow.netAfterTax, expensesWithPayout);
  const totalExpensesAll = expensesWithPayout;
  const liveMonthlyLeftoverNoPayout = incomeWithoutPayout - expensesWithoutPayout;
  ...
  const afterTaxPrincipalRequired =
    afterTaxEffectiveAnnualYield > 0 ? afterTaxTargetAnnualIncome / afterTaxEffectiveAnnualYield : 0;
  ...
  function addExpense() {
    ...
    setExpenses((previous) => [...previous, { id, name: "new expense", mode: "fixed", amount: "" }]);
  }
  ...
  function buildMonthlyRows(count) {
    ...
    for (let index = 0; index < safeCount; index += 1) {
      ...
      cashBalance += result.cashFlow.cashRemaining;
      ...
      rows.push({
        ...
        payoutUsed: result.cashFlow.payoutUsed,
        expenses: result.cashFlow.expenseTotal,
        reinvested: result.cashFlow.toReinvest,
        newDeployment: result.newDeployment,
        renewalDeployment: result.renewalDeployment,
        savedPool: result.savedPool,
        contracts: result.contracts,
        monthlySurplus: result.cashFlow.totalSurplus,
        cashRemaining: result.cashFlow.cashRemaining,
  ...
  function simulateNextYear() {
    ...
    let yearCashRemaining = 0;
    let yearSurplus = 0;
    let yearNewDeployments = 0;
    let yearRenewalDeployments = 0;
    ...
    for (let month = 0; month < 12; month += 1) {
      ...
      yearExpenses += result.cashFlow.expenseTotal;
      yearReinvest += result.cashFlow.toReinvest;
      yearPayoutUsed += result.cashFlow.payoutUsed;
      yearCashRemaining += result.cashFlow.cashRemaining;
      yearSurplus += result.cashFlow.totalSurplus;
      yearNewDeployments += result.newDeployment;
      yearRenewalDeployments += result.renewalDeployment;
    }
    ...
    setHistory((previous) => [
      ...previous,
      {
        ...
        cashRemaining: yearCashRemaining,
        newDeployments: yearNewDeployments,
        renewalDeployments: yearRenewalDeployments,
        contracts: ladderEnabled && currentLadderState ? currentLadderState.contracts.length : 1,
        savedPool: ladderEnabled && currentLadderState ? currentLadderState.savedPool : 0,
        ladderEnabled,
  ...
  function coverShortfall() {
    const shortfall = Math.max(0, expensesWithPayout - incomeWithoutPayout);
              ...
              <CardContent className="grid grid-cols-1 gap-4 p-4 md:grid-cols-3">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    <input id="manualTaxToggle" type="checkbox" checked={useManualTaxRate} onChange={(event) => setUseManualTaxRate(event.target.checked)} />
                  ...
                  {useManualTaxRate && (
                    <div>
                      ...
                      <SelectBox value={manualTaxRatePct.toString()} onChange={(value) => setManualTaxRatePct(Number(value))}>
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    <input id="stateTaxEnabled" type="checkbox" checked={stateTaxEnabled} onChange={(event) => setStateTaxEnabled(event.target.checked)} />
                  ...
                  {stateTaxEnabled && (
                    <div>
                      ...
                      <Input type="number" value={stateTaxPct} onChange={(event) => setStateTaxPct(Number(event.target.value) || 0)} />
                    </div>
                  )}
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" value={monthlyIncome} onChange={(event) => setMonthlyIncome(Number(event.target.value) || 0)} />

                  <Label>Other Income</Label>
                  <Input type="number" value={otherIncome} onChange={(event) => setOtherIncome(Number(event.target.value) || 0)} />

                  <Label>Expenses</Label>
                  <div className="rounded-xl bg-slate-50 p-3 text-xs text-slate-600">
                    Expenses are non-tax spending categories. Payout tax is withheld before after-tax payout is added to spendable income.
                  </div>

                  {expenses.map((expense) => (
                    <div key={expense.id} className="grid grid-cols-1 gap-2 rounded-xl border border-slate-200 p-2 sm:grid-cols-[1.4fr_0.9fr_0.8fr_auto] sm:items-center">
                      <Input type="text" value={expense.name} onChange={(event) => updateExpense(expense.id, { name: event.target.value })} />
                      <SelectBox value={expense.mode} onChange={(value) => updateExpense(expense.id, { mode: value })}>
                ...
                <div className="space-y-2">
                  ...
                  {payoutMode === "percent" && (
                    <div>
                      <Label>Percent of payout to mark used</Label>
                      <Input type="number" value={payoutPercent} onChange={(event) => setPayoutPercent(Number(event.target.value) || 0)} disabled={autoCoverShortfallEnabled} />
                    </div>
                  )}

                  {payoutMode === "dollar" && (
                    <div>
                      <Label>Monthly payout marked used, dollars</Label>
                      <Input type="number" value={payoutUsedMonthly} disabled={autoCoverShortfallEnabled} onChange={(event) => setPayoutUsedMonthly(Number(event.target.value) || 0)} />
                    </div>
                  )}
                  ...
                  <div className="rounded-xl bg-slate-50 p-3 text-sm">
                    ...
                    <div className="mt-1 text-xs text-slate-500">Reporting only. It does not get subtracted a second time from total surplus.</div>
                  </div>

                  <div className="flex items-center gap-2 pt-2">
                    <input id="autoCoverShortfall" type="checkbox" checked={autoCoverShortfallEnabled} onChange={(event) => setAutoCoverShortfallEnabled(event.target.checked)} />
                    <Label htmlFor="autoCoverShortfall">Auto cover wage-only shortfall with payout</Label>
                  </div>
                  ...
                  <SelectBox value={reinvestMode} onChange={setReinvestMode}>
                    ...
                    <option value="reinvest_all_total">Reinvest all total surplus</option>
                  ...
                  {reinvestMode === "reinvest_fixed" && (
                    <div>
                      ...
                      <Input type="number" value={reinvestFixedStr} onChange={(event) => setReinvestFixedStr(event.target.value)} />
                    </div>
                  )}
                  ...
                  {(reinvestMode === "reinvest_pct_of_payout" || reinvestMode === "reinvest_pct_of_total_surplus") && (
                    <div>
                      ...
                      <Input type="number" value={reinvestPctStr} onChange={(event) => setReinvestPctStr(event.target.value)} />
                    </div>
                  )}
                ...
                <CardContent className="space-y-4 p-4">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                    ...
                    <div>
                      ...
                      <Input type="number" value={monthsToSimulate} onChange={(event) => setMonthsToSimulate(Number(event.target.value) || 0)} />
                    </div>
                  ...
                  <div className="rounded-xl bg-slate-50 p-3 text-xs text-slate-600">
                    Monthly mode applies the selected reinvestment strategy every month. With contract laddering enabled, surplus enters the saved deployment pool, new contracts open when the pool reaches $10,000 in an available monthly slot, contracts age each month, and saved funds are added at renewal.
                  </div>
                </CardContent>
              </Card>
            )}
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
              <Card>
                <CardContent className="p-4">
                  ...
                  <div className="text-xs text-slate-500">Fixed and percentage spending only.</div>
                  <div className="text-xl font-semibold">{dollars(totalExpensesAll)}</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4">
                  <div className="text-sm text-slate-500">Monthly Payout Taxes</div>
                  <div className="text-xs text-slate-500">Held back before after-tax payout.</div>
                  <div className="text-xl font-semibold">{dollars(autoTaxesOwed)}</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4">
                    ...
                    <tbody>
                      {history.map((row) => (
                        <tr key={row.year} className="border-t">
                          ...
                          <td className="py-2 pr-4">{dollars(row.surplus)}</td>
                          <td className="py-2 pr-4">{dollars(row.reinvest)}</td>
                          <td className="py-2 pr-4">{dollars(row.payoutUsed)}</td>
                          <td className="py-2 pr-4">{dollars(row.cashRemaining)}</td>
                          <td className="py-2 pr-4">{dollars(row.newDeployments)}</td>
                          <td className="py-2 pr-4">{dollars(row.renewalDeployments)}</td>
          ...
          <Card>
            <CardContent className="space-y-6 p-4">
              <div>
                ...
                <Input type="number" value={converterPrincipal} onChange={(event) => setConverterPrincipal(Number(event.target.value) || 0)} />
              ...
              <div className="border-t pt-6">
                ...
                <Input type="number" value={converterHourly} onChange={(event) => setConverterHourly(Number(event.target.value) || 0)} />
              ...
              <div className="border-t pt-6">
                ...
                <Input type="number" value={converterAnnualSalary} onChange={(event) => setConverterAnnualSalary(Number(event.target.value) || 0)} />
              ...
              <div className="space-y-2 border-t pt-6">
                ...
                <Input type="number" value={afterTaxFedPct} onChange={(event) => setAfterTaxFedPct(Number(event.target.value) || 0)} />
                <Label>State tax %</Label>
                <Input type="number" value={afterTaxStatePct} onChange={(event) => setAfterTaxStatePct(Number(event.target.value) || 0)} />
                ...
                <Input type="number" value={afterTaxTargetIncome} onChange={(event) => setAfterTaxTargetIncome(Number(event.target.value) || 0)} />
